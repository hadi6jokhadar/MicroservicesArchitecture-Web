...
### 2d. URL-Synced List/Filter State (MANDATORY for every list/table page)

Every list/table page's filters and pagination must sync to the URL's query params, so a link to that exact filtered+paginated view can be shared/bookmarked, and browser back/forward works. Reference implementation: `apps/admin/src/app/features/translation/translations/translations.component.ts`.

- **Shared helper:** `@ihsan/core` exports `toQueryParams`, `updateQueryParams(router, route, values, replaceUrl = true)`, `queryParamNumber(map, key, fallback)`, `queryParamBoolean(map, key, fallback)` from `libs/core/src/lib/utils/query-params.util.ts`. These are the only shared pieces — no base class, no generic list controller. Everything else (which fields exist, their defaults, enum/sentinel coercion) stays hand-written per page, matching how `loadData()`'s query-object literal is already hand-written per page.
- **`ActivatedRoute.queryParamMap` is the sole fetch trigger**, not `ngOnInit()`. Angular's default route-reuse strategy keeps a list component alive across query-param-only navigations, so `ngOnInit` never reruns — a live `queryParamMap` subscription is the only mechanism that correctly covers initial load, in-app filter/page changes, and browser back/forward alike:
  ```typescript
  ...
  ```
  Remove any prior `ngOnInit()` load call and any `effect(() => { if (page > 1) loadData() })` page guard — both become unnecessary once this subscription exists.
- **`restoreFromQueryParams(map: ParamMap)`**: patches the filter `FormGroup` with `{ emitEvent: false }` and sets `currentPage` via `queryParamNumber(map, 'page', 1)` — silent, so it never re-triggers `writeStateToUrl()`.
- **`writeStateToUrl(replaceUrl = true)`**: reads `filterForm.getRawValue()` and calls `updateQueryParams(router, route, { page: currentPage() > 1 ? currentPage() : undefined, field: value || undefined, ... }, replaceUrl)`. Every field at its default (including a Zardui `'all'`/`'__all__'` sentinel) is passed as `undefined` so it's omitted from the URL — reuse the exact defaults already used in that page's own `onClearFilters()` reset literal. The page-number URL key is always the literal string `page`, regardless of what the backend DTO calls it (`pageNumber`, `page`, etc.) — map between them inside `loadData()`, same as any other field-name mismatch (`searchTerm` vs `textFilter`).
- **Existing triggers keep their exact conditions** (per-field `valueChanges`, `debounceTime`, an explicit search-button-only field) — only the terminal call changes from `loadData()` to `writeStateToUrl()`.
- **`replaceUrl` convention:** filter changes → `replaceUrl: true` (default; no history entry per keystroke). Page-number changes → `replaceUrl: false` (normal push, so back/forward pages through results).
- **Pagination binding:** use the explicit-handler form, not the two-way banana binding:
  ```html
  ...
  ```
- **Client-side-filtered pages** (a resolver preloads the full unfiltered dataset once; filtering/pagination is local `computed()` signals, e.g. `chat-sessions`, `token-usage-logs`): the `queryParamMap` subscription should call `restoreFromQueryParams(map)` only, NOT `loadData()` — the `computed()`s already react to the restored form/page state, and calling `loadData()` unconditionally would trigger a wasted (or, for page-only navigation, actively wrong) server refetch. If one field is genuinely server-only with no client-side equivalent filter (e.g. a `scope` selector that changes what the resolver/service fetches), compare its restored value against the value already held in the form before deciding whether to call `loadData()` — refetch only when it actually changed, so pure page-number navigation never triggers a network call. See `ai-settings.component.ts`/`ai-system-prompts.component.ts` for this pattern.
...
14. **`z-select` (`libs/ui/src/lib/zard/components/select/select.component.ts`) options rendered from an asynchronously-loaded data source (a signal populated later by an HTTP call) were not clickable — only options present at initial render worked.** The component wired each projected `z-select-item`'s click handler (`item.setSelectHost(...)`) inside `ngAfterContentInit()`, a **one-time** lifecycle hook, even though `contentChildren()` (the signal-based content query backing `selectItems()`) updates reactively as projected content changes. Any `z-select-item` that rendered *after* that hook already fired — e.g. a nasheed Songs page's Artist filter, whose options come from `@for (artist of artists(); ...)` where `artists` is a signal populated by `loadArtists()`'s HTTP response, arriving after the initial paint — kept `select` (its internal host reference) `null` forever, so `onClick()`'s `this.select()?.selectItem(...)` silently no-op'd with no console error. Static option lists (readonly arrays defined at class-construction time, e.g. `songStates`/`copyrightRiskLevels`) were already rendered by `ngAfterContentInit()` time and worked fine — masking this as a page-specific bug rather than a component one. **Fixed** by converting the one-time hook into a constructor `effect()` (`libs/ui/src/lib/zard/components/select/select.component.ts`) so the wiring re-runs every time `selectItems()`'s tracked signal changes, including after items are added asynchronously. **Whenever a `z-select`'s options come from a signal that isn't fully populated at first render (any async-loaded filter dropdown), verify the options are actually clickable, not just visually present** — a `z-select` bug like this fails completely silently.
