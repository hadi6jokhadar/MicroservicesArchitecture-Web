...
@Component({
  ...
  template: `
    <ng-container zardId="input-group" #z="zardId">
      @let addonBefore = zAddonBefore(); @if (addonBefore) {
      <div
        [class]="addonBeforeClasses()"
        [id]="addonBeforeId()"
        [attr.aria-disabled]="zDisabled() || zLoading()"
      >
        <ng-container *zStringTemplateOutlet="addonBefore">{{
          addonBefore
        }}</ng-container>
      </div>
      }
      ...
      @let addonAfter = zAddonAfter(); @if (addonAfter) {
      <div
        [class]="addonAfterClasses()"
        [id]="addonAfterId()"
        [attr.aria-disabled]="zDisabled() || zLoading()"
      >
        <ng-container *zStringTemplateOutlet="addonAfter">{{
          addonAfter
        }}</ng-container>
      </div>
      }
...
export class ZardInputGroupComponent {
  ...
  private readonly contentInput =
    contentChild<ZardInputDirective>(ZardInputDirective);
  ...
  protected readonly isAddonBeforeTemplate = computed(() =>
    isTemplateRef(this.zAddonBefore())
  );
  ...
  protected readonly addonAfterClasses = computed(() =>
    this.addonClasses('after')
  );
  protected readonly addonBeforeClasses = computed(() =>
    mergeClasses(
      this.addonClasses('before'),
      this.isAddonBeforeTemplate() ? 'pr-0.5' : ''
    )
  );
